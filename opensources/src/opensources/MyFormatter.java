 ...
 * @since 1.5
 */
public final class MyFormatter implements Closeable, Flushable {
	...
	private MyFormatter(Locale l, Appendable a) {
		this.a = a;
		this.l = l;
		this.zero = getZero(l);
	}

	private MyFormatter(Charset charset, Locale l, File file)
	 ...
	 * The locale used is the {@linkplain Locale#getDefault() default locale}
	 ...
	 */
	public MyFormatter(Appendable a) {
	 ...
	 *            is applied.
	 */
	public MyFormatter(Appendable a, Locale l) {
	 ...
	 *             creating the file
	 */
	public MyFormatter(String fileName) throws FileNotFoundException {
	 ...
	 */
	public MyFormatter(String fileName, String csn)
	 ...
	 *             If the named charset is not supported
	 */
	public MyFormatter(String fileName, String csn, Locale l)
			throws FileNotFoundException, UnsupportedEncodingException {
	 ...
	 */
	public MyFormatter(File file) throws FileNotFoundException {
		this(Locale.getDefault(Locale.Category.FORMAT), new BufferedWriter(
	 ...
	 *             If the named charset is not supported
	 */
	public MyFormatter(File file, String csn) throws FileNotFoundException,
	 ...
	 *             If the named charset is not supported
	 */
	public MyFormatter(File file, String csn, Locale l)
			throws FileNotFoundException, UnsupportedEncodingException {
	 ...
	 */
	public MyFormatter(PrintStream ps) {
	 ...
	 */
	public MyFormatter(OutputStream os) {
		this(Locale.getDefault(Locale.Category.FORMAT), new BufferedWriter(
	 ...
	 *             If the named charset is not supported
	 */
	public MyFormatter(OutputStream os, String csn)
			throws UnsupportedEncodingException {
	 ...
	 *             If the named charset is not supported
	 */
	public MyFormatter(OutputStream os, String csn, Locale l)
			throws UnsupportedEncodingException {
	 ...
	 * specified format string and arguments. The locale used is the one defined
	...
	private class FormatSpecifier implements FormatString {
		...
		private void printString(Object arg, Locale l) throws IOException {
			if (arg instanceof Formattable) {
				MyFormatter fmt = MyFormatter.this;
				if (fmt.locale() != l)
					fmt = new MyFormatter(fmt.out(), l);
		...
		// * @param precision
		// */
		// public void formatTo(MyFormatter formatter, int flags,
		...
		private void printHashCode(Object arg) throws IOException {
			String s = (arg == null ? "null" : Integer.toHexString(arg
					.hashCode()));
			print(s);
		}
		...
		private void print(long value, Locale l) throws IOException {
			...
			if (c == Conversion.DECIMAL_INTEGER) {
				...
				int len = (f.contains(Flags.ALTERNATE) ? s.length() + 1 : s
						.length());
				...
				int len = (f.contains(Flags.ALTERNATE) ? s.length() + 2 : s
						.length());
		...
		private void print(StringBuilder sb, double value, Locale l, Flags f,
			...
			if (c == Conversion.SCIENTIFIC) {
				...
				FormattedFloatingDecimal fd = new FormattedFloatingDecimal(
				...
				FormattedFloatingDecimal fd = new FormattedFloatingDecimal(
						value, prec, FormattedFloatingDecimal.Form.GENERAL);
				...
				if (exp != null) {
					...
					prec = prec - (value == 0 ? 0 : fd.getExponentRounded())
							- 1;
				}
		...
		private void print(StringBuilder sb, BigDecimal value, Locale l,
				Flags f, char c, int precision, boolean neg) throws IOException {
			if (c == Conversion.SCIENTIFIC) {
				...
				BigDecimalLayout bdl = new BigDecimalLayout(v.unscaledValue(),
						v.scale(), BigDecimalLayoutForm.SCIENTIFIC);
				...
				// requested precision.
				if (bdl.scale() == 0
						&& (f.contains(Flags.ALTERNATE) || nzeros > 0))
		...
		private Appendable print(StringBuilder sb, Calendar t, char c, Locale l)
			...
			case DateTime.HOUR: { // 'l' (1 - 12) -- like I
				...
				Flags flags = (c == DateTime.HOUR_OF_DAY_0
						|| c == DateTime.HOUR_0 ? Flags.ZERO_PAD : Flags.NONE);
		...
		private StringBuilder localizedMagnitude(StringBuilder sb,
				char[] value, Flags f, int width, Locale l) {
			if (sb == null)
				sb = new StringBuilder();
			int begin = sb.length();
